...
// может быть такое что кол-во обрабатываемых символов 0, тогда сразу выход (к примеру самое первое слово на стыке. до него пробелы)
        cmp     x0, #0 // trivial case
        bne     offset_ok
        mov     x0, x1 // out buffer
        b to_ret
offset_ok:
...
0:
    ...
    add     x19, x19, #1
    cmp     x10, #16
    bge     string_more_than_buffer // bge
